...
ts_diff = ts - 2*ts.shift(1) + ts.shift(2)
#ts_diff = ts.diff(2)#.diff()#.diff()
...
predictions_ARIMA_init0 = pd.Series(ts.ix[2]-ts.ix[1], index=ts.index)
...
#print predictions_ARIMA_diff_2_cumsum.head()
predictions_ARIMA_diff_1 = predictions_ARIMA_init0.add(predictions_ARIMA_diff_2_cumsum,fill_value=0)
  
predictions_ARIMA = -predictions_ARIMA_diff_1.shift(-1)
